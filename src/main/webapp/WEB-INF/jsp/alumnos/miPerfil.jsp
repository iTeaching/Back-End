...
<petclinic:layout pageName="Mi perfil">
	...
	<form>
			...
			<tbody>
				<c:forEach items="${listaSolicitada}" var="listaClase">
					<tr>
						...
						<td><c:out value="${listaClase.asignatura.precio}€" /></td>
						<td><c:choose>
								<c:when test="${listaClase.aceptacionAlumno==false}">
									<a class="button" href="/alumnos/aceptar/${listaClase.id}">Aceptar
										Clase</a>
								</c:when>
							</c:choose>
						
						<a class="button" href="/alumnos/cancelar/${listaClase.id}">Cancelar Clase</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		...
		<a class="button" href="/alumnos/nuevaClase">Solicitar
			nueva clase</a> <input value="Editar" class="btn btn-default"
			type="button"
			onclick="window.location.href='/alumnos/${alumno.id}/edit'">
	</form>
